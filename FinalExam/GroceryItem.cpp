...
namespace    // unnamed, anonymous namespace
{
  ...
  constexpr bool floating_point_is_equal( T const lhs,  U const rhs,  double const EPSILON = 1e-4 ) noexcept
  {
    ///////////////////////// TO-DO (1) //////////////////////////////
      ...
      // Take the absolute value of the difference between the two floating-point numbers and return whether it is less
      // or equal to EPSILON
...
///////////////////////// TO-DO (5) //////////////////////////////
{
  // Don't modify the object if it is already equal to the right-hand side object
  if (this != &rhs) {
...
{
  // Don't modify the object if it is already equal to the right-hand side object
  // Move the unnamed, temporary variable, instead of copying its attributes
  if (this != &rhs) {
...
///////////////////////// TO-DO (7) //////////////////////////////
{
  // No memory was allocated, so we do not delete anything
}
/////////////////////// END-TO-DO (7) ////////////////////////////
...
std::weak_ordering GroceryItem::operator<=>( const GroceryItem & rhs ) const noexcept
{
  ...
  // Create variables that will compare each attribute to its rhs counterpart
  // If the comparison returns 0, then the two attributes are equal and we move on to the next comparison
  // Or, we just return the weak_ordering result of making the comparison

  const std::weak_ordering firstComparison = _upcCode <=> rhs.upcCode();
  if (firstComparison != 0) return firstComparison;

  const std::weak_ordering secondComparison = _productName <=> rhs.productName();
  if (secondComparison != 0) return secondComparison;

  const std::weak_ordering thirdComparison = _brandName <=> rhs.brandName(); 
  if (thirdComparison != 0) return thirdComparison;

  const std::weak_ordering fourthComparison = std::weak_order(_price, rhs.price());
...
bool GroceryItem::operator==( const GroceryItem & rhs ) const noexcept
{
  ...
  ///////////////////////// TO-DO (20) //////////////////////////////
  // The price and the upcCode are the most likely to be different. Use floating_point_is_equal rather than ==
  return floating_point_is_equal(_price, rhs.price(), 1e-4) && _upcCode == rhs._upcCode && _productName == rhs._productName && _brandName == rhs._brandName;
  /////////////////////// END-TO-DO (20) ////////////////////////////
}
...
std::istream & operator>>( std::istream & stream, GroceryItem & groceryItem )
{
  ...
  // Create a local object first. From there, we will see if we set our groceryItem to our local item
  GroceryItem item;
  // Create a variable to "eat" the comma that divides the parameters. std::ws will ignore all white space until 
  // it stumbles a non-white character (the comma), so it needs to be cared of.
  char delimiterEater = '\0';

  // We want to ignore any white space between the different components (for instance: {"000034"  , "Reese"  }
  //                                                                                   {"000034","Reese"}  should be read the same)
  
  if (stream >> std::ws >> std::quoted(item._upcCode) && stream >> std::ws >> delimiterEater && delimiterEater == ',' &&
      stream >> std::ws >> std::quoted(item._brandName) && stream >> std::ws >> delimiterEater && delimiterEater == ',' &&
      stream >> std::ws >> std::quoted(item._productName) && stream >> std::ws >> delimiterEater && delimiterEater == ',' &&
      stream >> std::ws >> item._price) {
        // Set our groceryItem to the local item we created here
...
std::ostream & operator<<( std::ostream & stream, const GroceryItem & groceryItem )
{
  ...
  // Overloading ostream is much easier. String variables are put inside quotes, and all variables are separated by a comma
  // We don't forget the new line character at the end
