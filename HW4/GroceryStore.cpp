...
GroceryStore::GroceryStore( const std::string & persistentInventoryDB )
{
  ...
  // create a string variable representing the upcCode and an unsigned int variable representing the quantity of that item
  // in the store
  std::string upc;
  unsigned int quantity = 0;

  // read until end of file making sure that we remove any whitespace between the upcCode and the quantity
  while (fin >> std::ws >> std::quoted(upc) && fin >> std::ws >> quantity) {
...
GroceryStore::GroceryItemsSold GroceryStore::ringUpCustomers( const ShoppingCarts & shoppingCarts, std::ostream & receipt )
{
  ...
  for (const auto & customerShoppingCart : shoppingCarts) {
    // display the name of the shopping cart's owner
    receipt << customerShoppingCart.first << "'s shopping cart contains: \n";
    // ring up the customer's shopping cart
    auto purchasedGroceries = ringUpCustomer(customerShoppingCart.second, receipt);
    // merge the customer's purchased groceries into today's sales
    todaysSales.merge(purchasedGroceries);
  }
  /////////////////////// END-TO-DO (3) ////////////////////////////
...
GroceryStore::GroceryItemsSold GroceryStore::ringUpCustomer( const ShoppingCart & shoppingCart, std::ostream & receipt )
{
  ...
  ///////////////////////// TO-DO (4) //////////////////////////////
    ...
    ///
    ///
    /// Hint:  Here's some pseudo-code to get you started.
  ...
  for (const auto & pair : shoppingCart) {
    // store the upc in a string variable
    std::string upc = pair.first;

    // If the item is not found in the world wide grocery item database, indicate on the receipt it's free of charge
    auto item = worldWideGroceryDatabase.find(upc);

    if (item == nullptr) {
      std::string name = pair.second.productName();
      receipt << "    " << std::quoted(upc) << " (" << name << ") not found, the item is free!\n";
...
void GroceryStore::reorderItems( GroceryItemsSold & todaysSales, std::ostream & reorderReport )
{
  ...
  for (const auto & upc : todaysSales) {
    ...
    // If the grocery item is not in the store's inventory or if the number of grocery items on hand has fallen 
    // below the re-order threshold (REORDER_THRESHOLD)
    auto itemInInventory = _inventoryDB.find(upc);
    if (itemInInventory == _inventoryDB.end() || itemInInventory->second < REORDER_THRESHOLD) {
      // If the grocery item is not in the world wide grocery item database, display just the UPC
      if (auto item = worldWideGroceryDatabase.find(upc); item == nullptr) {
        reorderReport << " " << count << ":  {" << std::quoted(upc) << "\n";

      } else {
        // Display the grocery item's full description
        reorderReport << " " << count << ":  {" << *item << "}\n";
      }
      ++count;

      // If the grocery item is not in the store's inventory display a notice indicating the grocery item is no longer
      // sold in this store and will not be re-ordered
      if (itemInInventory == _inventoryDB.end()) {
        reorderReport << "        *** no longer sold in this store and will not be re-ordered\n\n";

      } else {
        // Display the current quantity on hand and the quantity re-ordered
        unsigned int quantity = _inventoryDB[upc];
        reorderReport << "        only " << quantity << " remain in stock which is "
                      << (REORDER_THRESHOLD - quantity) << " unit(s) below reorder threshold ("
                      << REORDER_THRESHOLD << "), re-ordering 20 more\n\n";
        
        // Increase the quantity on hand by the number of items ordered and received (LOT_COUNT)
