...
GroceryItemDatabase::GroceryItemDatabase( const std::string & filename )
{
  ...
  GroceryItem item;
  // read grocery items until end of file using GroceryItem's extraction operator
  while (fin >> item) {
    // insert them into _data using std::map's insert() function
    _data.insert({item.upcCode(), std::move(item)});
  }
  /////////////////////// END-TO-DO (2) ////////////////////////////
...
GroceryItem * GroceryItemDatabase::find( const std::string & upc ) {
  // delegate to std::map's find(), which returns a pair given an appropriate key
  std::map<std::string, GroceryItem>::iterator pair = _data.find(upc);
  // if the pair wasn't found within the data store, return a nullptr
  if (pair == _data.end()) return nullptr;
  // otherwise, return a reference to the second element of the pair, a grocery item
  return &pair->second;
}
