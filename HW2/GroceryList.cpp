...
std::size_t GroceryList::find( const GroceryItem & groceryItem ) const
{
  ...
  // return the distance from the beginning of the container till the item found to get the zero-based index of the item
  // in the container. If the element is not in the container, then it will return the distance from the beginning till the
  // the element after the last one, which is equal to the size.
  return std::distance(_gList_vector.cbegin(), std::find(_gList_vector.cbegin(), _gList_vector.cend(), groceryItem));
...
void GroceryList::insert( const GroceryItem & groceryItem, std::size_t offsetFromTop )        // insert provided grocery item at offsetFromTop, which places it before the current grocery item at offsetFromTop
{
  ...
  { /**********  Part 1 - Insert into array  ***********************/
    ///////////////////////// TO-DO (4) //////////////////////////////
      ...
      // if the size is greater than or equal to the capacity, throw CapacityExceeded_Ex
      if (size() >= _gList_array.size()) {
        throw CapacityExceeded_Ex("Grocery item inserted will not fit in capacity" exception_location);
      }

      // if the offset is equal to the last index, then we can just set the element at that index to the groceryItem,
      // without having to move anything
      if (offsetFromTop == _gList_array.size() - 1) {
        _gList_array[offsetFromTop] = groceryItem;
      } else {
        // move every element, from offsetFromTop to the end of the array, one additional spot
  ...
  { /**********  Part 2 - Insert into vector  **********************/
    ///////////////////////// TO-DO (5) //////////////////////////////
      ...
      // I could use std::next(), but I am choosing to use pointer arithmetic in this case
      // use std::vector::insert() by taking the iterator and the grocery item as parameters
  ...
  { /**********  Part 3 - Insert into doubly linked list  **********/
    ///////////////////////// TO-DO (6) //////////////////////////////
      ...
      //using std::next() to walk the list till the insertion point, and using std::list::insert()
  ...
  { /**********  Part 4 - Insert into singly linked list  **********/
    ///////////////////////// TO-DO (7) //////////////////////////////
      ...
      //before_begin() returns an iterator to to something BEFORE the first node, which is what we need here
...
void GroceryList::remove( std::size_t offsetFromTop )
{
  ...
  { /**********  Part 1 - Remove from array  ***********************/
    ///////////////////////// TO-DO (8) //////////////////////////////
      ...
      // update the size of the array after removing that element
      --_gList_array_size;
    /////////////////////// END-TO-DO (8) ////////////////////////////
  } // Part 1 - Remove from array
...
GroceryList & GroceryList::operator+=( const std::initializer_list<GroceryItem> & rhs )
{
  ///////////////////////// TO-DO (13) //////////////////////////////
    ...
    // for each item in the list, insert it to the bottom of each of our containers (array, vector, list, forward_list)
    for (auto && item : rhs) insert(item, Position::BOTTOM);
  /////////////////////// END-TO-DO (13) ////////////////////////////
...
GroceryList & GroceryList::operator+=( const GroceryList & rhs )
{
  ///////////////////////// TO-DO (14) //////////////////////////////
    ...
    // loop through the items of the parameter's list and insert each to the bottom of each container (array, vector, list, and forward_list)
    for (auto && item : rhs._gList_vector) insert(item, Position::BOTTOM);
  /////////////////////// END-TO-DO (14) ////////////////////////////
...
std::weak_ordering GroceryList::operator<=>( GroceryList const & rhs ) const
{
  ...
  ///////////////////////// TO-DO (15) //////////////////////////////
    ...
    /*
    // Option 1 - looping through the common extent of both grocery lists using iterators

    for (auto i = _gList_vector.cbegin(), j = rhs._gList_vector.cbegin();
         i != _gList_vector.cend() && j != rhs._gList_vector.cend(); 
         ++i, ++j) {
      // compare the items using the operator<=> overload from the GroceryItem interface
      // if the result is not equal to zero, return the comparison
      auto comparison = *i <=> *j;
      if (comparison != 0) {
        return comparison;
      }
    ...
    */
    ...
    // Option 2 - finding the common extent first, then looping till the common extent using indices
    size_t commonExtent = (size() <= rhs.size()) ? (size()) : (rhs.size());

    for (size_t i = 0; i < commonExtent; ++i) {
      auto comparison = _gList_vector[i] <=> rhs._gList_vector[i];
      if (comparison != 0) {
        return comparison;
      }
    }

    // compare and return the three-way comparison of the sizes of both lists
...
bool GroceryList::operator==( GroceryList const & rhs ) const
{
  ...
  ///////////////////////// TO-DO (16) //////////////////////////////
    ...
    // compare their sizes first, and if they are not equal, return false
    if (size() != rhs.size()) return false;

    // loop through each of the grocery items in both lists
    // if at one point they are not equal, we stop checking and return false
    // we use the operator== overload from the GroceryItem interface

    // Option 1 - with indices
    /*
    ...
    */
    ...
    // at this point, we checked that every item is equal to the rhs item, so return true
    return true;
  /////////////////////// END-TO-DO (16) ////////////////////////////
}
...
std::size_t GroceryList::gList_sll_size() const
{
  ///////////////////////// TO-DO (17) //////////////////////////////
    ...
    // using std::distance() from the beginning node till the end node
...
std::istream & operator>>( std::istream & stream, GroceryList & groceryList )
{
  ...
  // doing while (stream >> item) will check for both eof() and bad(), and we are using the operator>> overload
  // from GroceryItem to read new items.
  while (stream >> item) {
    // insert the item to the bottom of our groceryList
    groceryList.insert(std::move(item), GroceryList::Position::BOTTOM);
  }
  /////////////////////// END-TO-DO (18) ////////////////////////////
